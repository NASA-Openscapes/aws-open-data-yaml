get_tutorials <- function(tutorials_df, shortname) {
  tutorials <- tutorials_df |>
    dplyr::filter(.data$shortname == .env$shortname) |>
...
get_metadata <- function(
  ...
  items <- httr2::resp_body_json(resp)$items
...
update_tag_counts <- function(tags, csv_file) {
  ...
  for (tag in tags) {
    if (tag %in% tag_df$tag) {
      ...
      tag_df <- dplyr::bind_rows(tag_df, data.frame(tag = tag, n = 1L))
    }
  }

  # Write updated data frame back to CSV
  readr::write_csv(tag_df, csv_file, append = FALSE)
}

get_contact_info <- function(umm) {
  contact_groups <- purrr::compact(purrr::map(umm$DataCenters, \(x) {
    x[["ContactGroups"]]
  }))

  cgs <- purrr::map_chr(contact_groups, \(x) {
    group_name <- x[[1]]$GroupName
    email <- purrr::map_chr(
      purrr::keep(x[[1]]$ContactInformation$ContactMechanisms, \(y) {
        y$Type == "Email"
      }),
      "Value"
    )
...
get_publications <- function(umm) {
  ...
  purrr::map(umm$PublicationReferences, \(pub) {
    doi <- pub$DOI[[1]]
...
write_nasa_aws_yaml <- function(shortname, tutorials_df, dir) {
  ...
  yaml::write_yaml(
    purrr::compact(yaml_data),
    file = file.path(
      dir,
      paste0("nasa-", slugify::slugify(shortname), ".yaml")
    ),
    indent.mapping.sequence = TRUE,
    handlers = list(logical = yaml::verbatim_logical, character = trimws)
  )
}
